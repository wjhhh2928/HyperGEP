...
from models.env_encoder import EnvHGAT
from models.snp_encoder import SNPEncoderCNN
...
def parse_args():
    ...
    parser.add_argument("--wandb_project", type=str, default="HyperGEP-MetaFusion")
...
def main():
    ...
    for i in range(num_traits):
        ...
        for epoch in range(args.pretrain_epochs):
            ...
            wandb.log({f"Pretrain/Trait_{i}_Loss": epoch_loss / len(train_loader), "epoch": epoch})
    ...
    y_true = torch.tensor(phe[:, args.target_idx], dtype=torch.float32).to(device)

    # 5. 阶段二：Meta-Fusion 5 折交叉验证
    print("\n>>> Meta-Fusion 5-Fold Cross Validation <<<")
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(np.arange(fused.shape[0]))):
        ...
        for epoch in range(args.meta_epochs):
            ...
            wandb.log({
                f"Fold_{fold_id}/Loss": loss.item(),
                f"Fold_{fold_id}/PCC": pear,
                f"Fold_{fold_id}/R2": r2,
                "epoch": epoch
            })
        ...
        print(f"[Fold {fold_id}] Best PCC: {best_pcc:.4f} | R2: {best_r2:.4f}")
    ...
    df_metrics.to_csv("meta_fusion_5fold_metrics.csv", index=False)
    
    visualize_trait_weights(best_weights, "meta_attention_weights.png")
    wandb.save("meta_fusion_5fold_metrics.csv")
    wandb.log({"Meta_Attention_Weights": wandb.Image("meta_attention_weights.png")})
    wandb.finish()
